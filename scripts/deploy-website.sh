...
extract_cdk_output() {
  # note, relies on order of outputs since they have to have a unique name per stack within the same account and thus they can't be hardcoded here
  S3_BUCKET_NAME=$(cat $CDK_OUTPUT_FILE | jq -r 'nth(0; to_entries[] | .value) | nth(0; to_entries[] | .value)')
  DISTRIBUTION_ID=$(cat $CDK_OUTPUT_FILE | jq -r 'nth(0; to_entries[] | .value) | nth(1; to_entries[] | .value)')
}
